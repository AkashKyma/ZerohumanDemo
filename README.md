# DEM-6 Storefront with Product Management
...
The app now includes the original shopping flow, the DEM-3 marketing shell, and the DEM-6 product-management experience:

- **Homepage** with hero content and featured products
- **Catalog page** with shopper-facing browsing for active products
- **Product detail pages** for individual storefront items
...
- **Active/inactive toggle** so hidden products stay out of shopper-facing catalog and detail views
- **Browser-local catalog persistence** so management changes survive refreshes in the current browser
- **Automated tests** covering cart behavior, checkout behavior, product-management utilities, and storefront detail visibility for inactive products

## DEM-6 feature overview

The ecommerce app now supports managed products with these fields:
...
- Inactive products are also hidden from shopper-facing product detail experiences
...
- `/products/manage` — product management dashboard added for DEM-6
...
- `src/__tests__/product-detail-client.test.tsx`
...
Documentation, changelog, and implementation notes have been updated for DEM-6. Current release-readiness checks should include:
